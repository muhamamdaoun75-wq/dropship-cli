...
const config = {
  ...
  getShopifyApiKey() {
    return store.get('shopifyApp.apiKey') || process.env.SHOPIFY_API_KEY || null
  },
  getShopifyApiSecret() {
    return store.get('shopifyApp.apiSecret') || process.env.SHOPIFY_API_SECRET || null
  },
  setShopifyApp({ apiKey, apiSecret }) {
    if (apiKey) store.set('shopifyApp.apiKey', apiKey)
    if (apiSecret) store.set('shopifyApp.apiSecret', apiSecret)
  },
  hasShopifyApp() {
    return !!(this.getShopifyApiKey() && this.getShopifyApiSecret())
  },
  ...
  summary() {
    return {
      ...
      shopifyAppConfigured: this.hasShopifyApp()
    }
  },
